...
用户要求避免画面变化就整屏 OCR。新增会话内 MessageState，以原生控件 RuntimeId + 原文保存已确认方向；
基于已知尾项判断追加，重复文字但不同 ID 仍是新项，旧历史不被当成新消息。
稳定两次后才处理；未知方向的新项保留横向坐标，仅裁其上下范围作为一批发给 MinerU，
校验位置和布局后上传。区域重叠、缺失、出屏时暂停，不退回全屏截图。原文不被 OCR 改写。

初始人工确认直接建立缓存，取消此前冗余首帧 OCR 对照；面板的自动预选仍一次全消息区 OCR。
消息不变只本地查询；己方末句不生成，生成前后确认原生快照。停止/角色/内容变化取消独立后台任务，
自动内容不写长期记忆。每会话最多 500 条缓存，均不落盘。

109 项本地测试通过，新增 15 项缓存/裁剪/调用次数/旧结果/停止联动测试。
实机 RuntimeId 和矩形稳定性尚未验证；RuntimeId 不是服务端消息 ID，同 ID 同文本复用仍无法可靠检测。
连续到达的新消息可能使在途识别重试，不承诺绝对每条一次 OCR；后台、群聊、媒体完整性不在本次支持范围。
